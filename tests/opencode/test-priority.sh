...
# Verifies that skills are resolved with correct priority: project > personal > wolfpowers
...
# 1. Create in wolfpowers location (lowest priority)
mkdir -p "$HOME/.config/opencode/wolfpowers/skills/priority-test"
cat > "$HOME/.config/opencode/wolfpowers/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Wolfpowers version of priority test skill
---
# Priority Test Skill (Wolfpowers Version)

This is the WOLFPOWERS version of the priority test skill.

PRIORITY_MARKER_WOLFPOWERS_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/wolfpowers/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Wolfpowers version exists"
else
    echo "  [FAIL] Wolfpowers version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides wolfpowers
echo ""
echo "Test 2: Testing personal > wolfpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides wolfpowers)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_WOLFPOWERS_VERSION"; then
    echo "  [FAIL] Wolfpowers version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|wolfpowers" | head -10
fi

# Test 3: Test that project overrides both personal and wolfpowers
echo ""
echo "Test 3: Testing project > personal > wolfpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Wolfpowers version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit wolfpowers: prefix bypasses priority
echo ""
echo "Test 4: Testing wolfpowers: prefix forces wolfpowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load wolfpowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_WOLFPOWERS_VERSION"; then
    echo "  [PASS] wolfpowers: prefix correctly forces wolfpowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] wolfpowers: prefix did not force wolfpowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
