...
# Create skills in personal and wolfpowers locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/wolfpowers-skills/shared-skill"
mkdir -p "$TEST_HOME/wolfpowers-skills/unique-skill"
...
cat > "$TEST_HOME/wolfpowers-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: Wolfpowers version
---
# Wolfpowers Shared
EOF

cat > "$TEST_HOME/wolfpowers-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in wolfpowers
---
# Unique
EOF
...
function resolveSkillPath(skillName, wolfpowersDir, personalDir) {
    const forceWolfpowers = skillName.startsWith('wolfpowers:');
    const actualSkillName = forceWolfpowers ? skillName.replace(/^wolfpowers:/, '') : skillName;

    if (!forceWolfpowers && personalDir) {
    ...
    if (wolfpowersDir) {
        const wolfpowersPath = path.join(wolfpowersDir, actualSkillName);
        const wolfpowersSkillFile = path.join(wolfpowersPath, 'SKILL.md');
        if (fs.existsSync(wolfpowersSkillFile)) {
            return {
                skillFile: wolfpowersSkillFile,
                sourceType: 'wolfpowers',
                skillPath: actualSkillName
            };
        }
    }
...
const wolfpowersDir = '$TEST_HOME/wolfpowers-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', wolfpowersDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: wolfpowers: prefix should force wolfpowers
const forced = resolveSkillPath('wolfpowers:shared-skill', wolfpowersDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to wolfpowers
const unique = resolveSkillPath('unique-skill', wolfpowersDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', wolfpowersDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow wolfpowers skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"wolfpowers"'; then
    echo "  [PASS] wolfpowers: prefix forces wolfpowers resolution"
else
    echo "  [FAIL] wolfpowers: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"wolfpowers"'; then
    echo "  [PASS] Unique wolfpowers skills are found"
else
    echo "  [FAIL] Unique wolfpowers skills not found"
    exit 1
fi
